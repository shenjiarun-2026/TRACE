...
def compute_all_model_descriptors(args):
    ...
    for i, model_path in enumerate(model_paths):
        ...
        print(f"[TRACE] Loading model {model_name}: {model_path}")
        ...
        for data_path in dataset_files:
            print(f"[TRACE] Descriptor computation: model={model_name} data={data_path}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train TRACE global metric weights.")
    ...
    parser.add_argument("--weights_output", default="trace_global_weights.json")
    parser.add_argument("--stability_output", default="trace_stability_report.json")
...
def main():
    ...
    print(f"[TRACE] Pooled training rows: {len(y)}; features: {len(names)}")
    ...
    print("[TRACE] Learned global weights:")
    for key, value in sorted(weights.items(), key=lambda x: -abs(x[1])):
        print(f"  {key:24s} {value:+.4f}")
    print(f"[TRACE] CV acc={perf['acc_mean']:.3f}±{perf['acc_std']:.3f}; f1={perf['f1_mean']:.3f}±{perf['f1_std']:.3f}")
