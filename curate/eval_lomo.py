...
def main():
    parser = argparse.ArgumentParser(description="Skeleton helper for CURATE leave-one-model-out evaluation.")
    ...
    report = {
        "note": "Run curate.train once per holdout model by filtering rankings externally or extend this skeleton.",
        "models_in_rankings": models,
        "folds": [{"holdout_model": m, "train_models": [x for x in models if x != m]} for m in models],
    }
    write_json(args.output, report)
    print(f"[CURATE] Wrote LOMO plan to {args.output}")
